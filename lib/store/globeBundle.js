...
const globeBundle = {
   ...
   * @param {String} options.ionAccessToken - Ion token for Cesium Ion
   * @param {String} options.esriAccessToken - ESRI token for ESRI services
   * @param {Position} options.initialPosition
   * @param {Function} options.onMount - Callback function to be called when the viewer is mounted
   * @returns undefined
   */
  doGlobeInitialize:
    ({
    ...
    async ({ dispatch, store }) => {
      ...
      // if you provide terrain or terrainProvider, use those,
      // otherwise if ESRI token is available add it,
      // otherwise look for the ionAccessToken,
      // otherwise use the default ellipsoid terrain
      if (opts.terrainProvider) {
        viewer.terrainProvider = opts.terrainProvider;
      } else if (opts.terrain) {
        viewer.terrain = opts.terrain;
      } else if (esriAccessToken) {
        ...
        terrain.readyEvent.addEventListener((provider) => {
          console.log("Using Cesium Ion terrain");
